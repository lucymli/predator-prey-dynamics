...
par <- c(preyGrow = 1,		#intrinsic growth rate of prey
		ratePred = 0.2,			#rate of predation
		assimEff = 0.01,		#assimilation efficiency
		predDeath = 0.5,			#intrinsic death rate of predator
		handTime = 0.0005		#handling time
)

# 1b. Parameter to be changed
parChange <- seq(1,2,0.2)		#assessing the effect of r values between 0 and 2 on extinction risk and mean extinction time

# 2. Define system
state <- c(N=10, P=10)        # Initial state vector
stateChange <- matrix(c(+1, -1, -1, 0, 0, 0, +1, -1),nrow=2,byrow=T) # State-change matrix
a  <- c("preyGrow*N", "ratePred*N*P/(1+ratePred*handTime*N)","assimEff*ratePred*N*P/(1+ratePred*handTime*N)","predDeath*P") # Propensity vector  
tf <- 10         # Final time
...
#Plots results
par(mfrow=c(1,2))
plot(result[,1],result[,2],type="n",ylim=c(0,1),xlab="Prey Growth Rate",ylab="Probability of Extinction")
...
write.csv(result,"/Users/lmq/Documents/IC/Final Year Project/Reports/Results/2. Stochastic Predator Prey Models/Data. 2k.csv")