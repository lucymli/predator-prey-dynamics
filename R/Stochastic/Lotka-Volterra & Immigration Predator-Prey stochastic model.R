...
par <- c(preyGrow = 0.5,		#intrinsic growth rate of prey
		...
		rateImmPrey = 10,		#rate of immigration for prey
		rateImmPred = 10		#rate of immigration for predator
)

# 1b. Parameter to be changed
parChange <- seq(0.0,2,0.2)		#assessing the effect of r values between 0 and 2 on extinction risk and mean extinction time

# 2. Define system
state <- c(N=10, P=10)        # Initial state vector
stateChange <- matrix(c(+1, -1, -1, 0, +1, 0, 0, 0, +1, -1, 0, +1),nrow=2,byrow=T) # State-change matrix
a  <- c("preyGrow*N", "ratePred*N*P","assimEff*ratePred*N*P","predDeath*P","rateImmPrey","rateImmPred") # Propensity vector  
tf <- 10         # Final time
...
colnames(result) <- c("r","Prey Extinction Probability","Mean Prey Extinction Time","Predator Extinction Probability","Mean Predator Extinction Time")		#set dataframe column headings
...
plot(result[,1],result[,5],type="n",xlab="Prey Growth Rate, r",ylab="Mean Time to Extinction",ylim=c(0,12.16))
lines(result[,1],result[,3],col="blue")
lines(result[,1],result[,5],col="red")
legend("topright",c("Prey","Predator"),col=c("blue","red"),lty=1)

#Exports results as csv file
write.csv(result,"/Users/lmq/Documents/IC/Final Year Project/Reports/Results/3. Effect of immigration/Data. 3b.csv")