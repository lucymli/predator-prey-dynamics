#===============================================================================
# Lotka Volterra with Density Dependence predator-prey model (Gillespie, 1977; Kot, 2001)
#===============================================================================
...
parChange <- seq(0.0,2,0.1)		#assessing the effect of r values between 0 and 2 on extinction risk and mean extinction time

# 2. Define system
state <- c(N=10, P=10)        # Initial state vector
stateChange <- matrix(c(+1, -1, -1, 0, 0, 0, +1, -1),nrow=2,byrow=T) # State-change matrix
a  <- c("preyGrow*N*(1-N/carryCap)", "ratePred*N*P","assimEff*ratePred*N*P","predDeath*P") # Propensity vector  
tf <- 10         # Final time
numRuns <- 10000		#number of runs of the stochastic model for each parameter value
...
alpha <- matrix(data = NULL, nrow=length(parChange),ncol=1)		#Prey extinction probability matrix
beta <- matrix(data = NULL, nrow=length(parChange),ncol=1)		#Mean prey extinction time
gamma <- matrix(data = NULL, nrow=length(parChange),ncol=1)		#Predator extinction probability
delta <- matrix(data = NULL, nrow=length(parChange),ncol=1)		#Mean predator extinction time

#'for' loop for calculating prey/predator extinction probabilities and mean prey/predator extinction times for each parameter value
...
colnames(result) <- c("d","Prey Extinction Probability","Mean Prey Extinction Time","Predator Extinction Probability","Mean Predator Extinction Time")		#set dataframe column headings


#Plots results
par(mfrow=c(1,2))
plot(result[,1],result[,2],type="n",ylim=c(0,1),xlab="Predator Death Rate, d",ylab="Probability of Extinction")
lines(result[,1],result[,2],col="blue")
lines(result[,1],result[,4],col="red")
legend("topright",c("Prey","Predator"),col=c("blue","red"),lty=1)
plot(result[,1],result[,5],type="n",xlab="Predator Death Rate, d",ylab="Mean Time to Extinction",ylim=c(0,7))
lines(result[,1],result[,3],col="blue")
lines(result[,1],result[,5],col="red")
legend("topright",c("Prey","Predator"),col=c("blue","red"),lty=1)

#Exports results as csv file
write.csv(result,"/Users/lmq/Documents/IC/Final Year Project/Reports/Results/2. Stochastic Predator Prey Models/Data. 2h.csv")