###Deterministic Lotka-Volterra Model with Density Dependence####
...
par <- c(preyGrow = 0.5, 		# intrinsic growth rate of prey
		...
		carryCap = 1*pop		# carrying capacity of the system - set to be a multiple of the initial prey population size
		)
...
LVDetDD <- function(Time, State, Param){
	with(as.list(c(State, Param)), {
		dXdt <- preyGrow*Prey*(1-Prey/carryCap) - ratePred*Prey*Pred			#Change in prey population is dependent on prey birth and prey death due to predation; prey birth rate is density dependent
		dYdt <- assimEff*ratePred*Prey*Pred - predDeath*Pred					#Change in predator population is dependent on predator birth (from assimilation of prey) and predator death
...
matplot(out[ , 2],out[ , 3],type="l",xlab = "Prey", ylab="Predator",main="K = N",lty=1,lwd=2,col="Blue")

#Plots time evolution of prey and predator
matplot(out[ , 1], out[ , 2:3], type = "l", xlab = "Time", ylab = "Population size",
main = "K = N", lty=1,lwd = 2,col = c("Blue","Red"))
