###Deterministic Lotka-Volterra Model with Density-Dependence AND Type 2 Functional Response####
...
par <- c(preyGrow = 0.5, 		# intrinsic growth rate of prey
		...
		carryCap = 50*pop,			# carrying capacity
		handleTime = 0.05		# handling time
		)
...
LVDetDD <- function(Time, State, Param){
	with(as.list(c(State, Param)), {
		dXdt <- preyGrow*Prey*(1-Prey/carryCap) - ratePred*Prey*Pred/(1+ratePred*handleTime*Prey)			#Change in prey population is dependent on prey birth and prey death due to predation; prey birth rate is density dependent
...
matplot(out[ , 2],out[ , 3],type="l",xlab = "Prey", ylab="Predator",main="K = 500; h = 5e-2",lty=1,lwd=2,col="Blue")

#Plots time evolution of prey and predator
matplot(out[ , 1], out[ , 2:3], type = "l", xlab = "Time", ylab = "Population size",
main = "K = 500; h = 5e-2", lty=1,lwd = 2,col = c("Blue","Red"))
